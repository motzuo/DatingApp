...
namespace API.Data
{
    public class LikesRepository : ILikesRepository
    {
        ...
        public async Task<PagedList<LikeDto>> GetUserLikes(LikesParams likesParams)
        {
            ...
            var users = likesParams switch
            {
                { Predicate: "liked" } => likes.Where(like => like.SourceUserId == likesParams.UserId).Select(like => like.LikedUser),
                _ => likes.Where(like => like.LikedUserId == likesParams.UserId).Select(like => like.SourceUser)
            };
